...
Answer: The relationship is a relational one. However, no foreign keys directly relate these tables. There is an intermediate table called EmployeeTerritory that includes both employee and territory IDs. This intermediate table is used to link info from employee and territory.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

Answer: Using a document store can be useful for several reasons: a less restrictive schema, large/everchanging data, and horizontal computing. A less restrictive schema means you can put multi-line transactions(e.g. accounting), it also means syntax and document structure can vary within that database. Horizontal computing is sharding(adding more servers), thus distributing the load across devices.

- What is "NewSQL", and what is it trying to achieve?

Answer: It is a grouping(category) of technologies aiming to split the difference between SQL and NoSQL. Essentially traditional SQL databases, they try to accomadate some of the short comings of old SQL. Each technology seems to cover old SQL relationals, while trying to implement one aspect of NoSQL. Some choose unusual data schemas, some choose scalability, etc. 
